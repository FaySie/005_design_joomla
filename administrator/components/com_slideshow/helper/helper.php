...
abstract class SlideshowHelper
{
	...
	public static function addSubmenu($vName)
	{
		...
		foreach (new \DirectoryIterator(JPATH_ADMINISTRATOR . '/components/com_slideshow/view') as $folder)
		{
			if ($folder->isDir() && $inflector->isPlural($view = $folder->getBasename()))
			{
				JHtmlSidebar::addEntry(
					JText::sprintf(sprintf('COM_SLIDESHOW_%s_TITLE_LIST', strtoupper($folder))),
					'index.php?option=com_slideshow&view=' . $view,
					($vName == $view)
				);
			}
		}
		...
		$dispatcher->trigger('onAfterAddSubmenu', array('com_slideshow', $vName));
	}
	 ...
	 * @return  JObject
	 */
	public static function getActions($option = 'com_slideshow')
	{
